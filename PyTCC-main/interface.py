...
#Buttons3
button3 = tk.Button(file_frame, text='Negativo', font='Georgia', command=lambda: filter_neg())
button3.place(rely=0.100, relx=0.100)

button4 = tk.Button(file_frame, text='Positivo', font='Georgia', command=lambda: filter_posi())
button4.place(rely=0.300, relx=0.100)

button5 = tk.Button(file_frame, text='Conciliado', font='Georgia', command=lambda: filter_conc())
...
button2 = tk.Button(file_frame, text='Calcular', font='Georgia', command=lambda: view_Excel())
button2.place(rely=0.65, relx=0.20)

button6 = tk.Button(file_frame, text='Exportar', font='Georgia', command=lambda: export())
...
def intoNumber(file, string, string1):
   file = file.dropna(how='all', subset=['Transação','Saldo']) # drop all the rows with null values      
   file = file.fillna({string:0, string1 :0}).fillna(0) # replace the values NaN to 0 in columns ''
   file[[string, string1]] = (file[[string, string1]].apply(pd.to_numeric)).astype(float) #converter todos as nossas colunas desejadas no tipo float   
   return file
...
def view_Excel():    
    ...
    csv_df = pd.read_csv(file_path, encoding='latin-1', sep=';', decimal=',')   # read csv   
...
def filter_neg():
    ...
    newIndex = novo_csvdf.sort_values(by=['Conciliação']) 
    newIndexNoDuplicates = newIndex.drop_duplicates()   
    newnew = (newIndexNoDuplicates.query(' Conciliação < 0'))
...
def export():
    ...
    try:
       ...
       df.to_csv('Analiticos.csv', index=False) #assim ele cria um arquivo novo na raiz do projeto, porem ele sobrescreve sempre que é exportado
    except ValueError:
        tk.messagebox.showerror('Information', 'the file you have choose is invalid')
        return None
    except FileNotFoundError:
